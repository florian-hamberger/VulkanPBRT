...
{
public:
    TerrainAccelerationStructureManager(vsg::ref_ptr<vsg::Context> context, const vsg::Path& heightmapPath, const vsg::Path& texturePath, float terrainScale, float terrainScaleVertexHeight, bool terrainFormatLa2d, bool textureFormatS3tc, int tileLengthLodFactor, int lodLevelCount);
    std::pair<vsg::ref_ptr<vsg::TopLevelAccelerationStructure>, vsg::ref_ptr<vsg::Node>> createTlasAndScene(vsg::dvec3 eyePosInTileCoords, int lodViewDistance, bool maxLod);
    std::vector < vsg::ref_ptr<vsg::TopLevelAccelerationStructure>> buildAllLodTlas();
